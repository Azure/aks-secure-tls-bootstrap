...
func run(ctx context.Context) int {
	...
	bootstrapCtx := telemetry.WithTaskRecorder(ctx, telemetry.NewTaskRecorder())
	bootstrapCtx, cancel := context.WithDeadline(bootstrapCtx, bootstrapDeadline)
	defer cancel()

	finalErr, bootstrapErrors, bootstrapRecording := bootstrap.PerformBootstrapping(bootstrapCtx, logger, bootstrapClient, bootstrapConfig)
	...
	bootstrapResult := &bootstrap.Result{
		...
		BootstrapRecording:  bootstrapRecording,
	}
